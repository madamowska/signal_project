...
public class BloodOxygenAlertService implements AlertService {
    ...
    public Alert checkAndTriggerAlerts (Patient patient, List<PatientRecord> records, AlertGenerator alertGenerator) {
        for (int i = 0; i < records.size(); i++) {
            ...
            if ("Saturation".equals(current.getRecordType())) {
                ...
                if (currentSaturation < MINIMAL_SATURATION) {
                    return new Alert(Integer.toString(current.getPatientId()), "Low blood oxygen saturation detected: " + currentSaturation*100 + "%", current.getTimestamp());
                ...
                if(records.size()>1 && i<records.size()-1){
                    ...
                    if (timeDiff <= TEN_MINUTES && (currentSaturation - nextSaturation) >= MAXIMUM_SATURATION_DROP) {
                        return new Alert(Integer.toString(current.getPatientId()), "Rapid drop in blood oxygen saturation detected.", current.getTimestamp());
