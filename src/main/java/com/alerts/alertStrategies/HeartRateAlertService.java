        ...
        for (PatientRecord record : records) {
            if ("HeartRate".equals(record.getRecordType())) {
                ...
                if (heartRate < MIN_HEART_RATE || heartRate > MAX_HEART_RATE) {
                    alertGenerator.triggerAlert(new SimpleAlert(Integer.toString(record.getPatientId()), "Abnormal heart rate detected: " + heartRate, record.getTimestamp()),patient);
                }
                ...
                if (previousRecord != null) {
                    ...
                    if (variation > VARIABILITY_THRESHOLD) {
                        alertGenerator.triggerAlert(new SimpleAlert(Integer.toString(record.getPatientId()),
